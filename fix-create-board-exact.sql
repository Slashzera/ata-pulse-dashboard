-- Correção EXATA baseada no erro do Supabase
-- Erro: Could not find the function public.create_board_with_type(background_color, board_title, board_type_uuid, company, process_number, process_value, responsible_person)
...
-- 2. Criar função com EXATAMENTE os parâmetros que o erro mostra
...
DECLARE
    ...
    new_board_id UUID;
BEGIN
    -- Inserir novo quadro (usando valores padrão para campos não enviados)
    INSERT INTO trello_boards (
        ...
        board_title,
        'Descrição do processo: ' || board_title, -- Valor padrão para description
        background_color,
        auth.uid(),
        board_type_uuid,
        process_number,
        responsible_person,
        company,
        'Objeto: ' || board_title, -- Valor padrão para object_description
        process_value
...
-- 3. Verificar se a função foi criada com a assinatura EXATA
SELECT 
...
-- 4. Mostrar a assinatura esperada vs criada
SELECT 'ERRO ESPERAVA:' as tipo, 'background_color, board_title, board_type_uuid, company, process_number, process_value, responsible_person' as parametros
UNION ALL
SELECT 'FUNÇÃO CRIADA:' as tipo, pg_get_function_arguments(p.oid) as parametros
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE p.proname = 'create_board_with_type'
AND n.nspname = 'public';