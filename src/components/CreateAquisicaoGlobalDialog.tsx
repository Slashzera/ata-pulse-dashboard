...
interface CreateAquisicaoGlobalDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

const CreateAquisicaoGlobalDialog: React.FC<CreateAquisicaoGlobalDialogProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Nova Aquisição Global</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              {createAtaMutation.isPending ? 'Salvando...' : 'Criar Aquisição Global'}
            </Button>
          </div>
        </form>
      </DialogContent>
...
export default CreateAquisicaoGlobalDialog;
