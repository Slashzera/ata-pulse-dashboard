-- Sistema Completo Trellinho - Funcionalidades Avançadas
...
BEGIN
    ...
    PERFORM check_automations(card_uuid, 'card_moved', 
                             json_build_object('from_list', old_list_id, 'to_list', target_list_uuid));
...
CREATE OR REPLACE FUNCTION check_automations(
    card_uuid UUID,
...
DECLARE
    ...
    board_uuid UUID;
BEGIN
    -- Obter board do cartão
    SELECT b.id INTO board_uuid
    FROM trello_cards c
    JOIN trello_lists l ON c.list_id = l.id
    JOIN trello_boards b ON l.board_id = b.id
    WHERE c.id = card_uuid;

    -- Buscar automações ativas para este board e trigger
    FOR automation IN 
        SELECT * FROM trello_automations 
        WHERE board_id = board_uuid 
        AND trigger_type = check_automations.trigger_type 
        AND is_active = TRUE
    LOOP
        -- Executar ação baseada no tipo
        CASE automation.action_type
            WHEN 'move_card' THEN
                PERFORM move_card_to_list(
                    card_uuid, 
                    (automation.action_parameters->>'target_list_id')::UUID
                );
            WHEN 'add_label' THEN
                PERFORM manage_card_labels(
                    card_uuid,
                    ARRAY[(automation.action_parameters->>'label_id')::UUID]
                );
        END CASE;
    END LOOP;
END;
